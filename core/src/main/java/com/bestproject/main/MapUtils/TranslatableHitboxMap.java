...
import com.bestproject.main.ObjectFragment.TranslatableHitbox;

public class TranslatableHitboxMap{
    boolean flipY;
    boolean flipX;
    boolean rotated90deg;
    float translationX, translationY;
    public Array<TranslatableHitbox> hitboxArray;
    public Array<Array<Array<Integer>>> hitboxMap=new Array<>();
    public float tileSize = 2f;
    int xRes, yRes;
    float minX,minY,maxX,maxY;
    public TranslatableHitboxMap(Array<HITBOX> hitboxArray) {
        minY=10000;
        minX=10000;
        maxY=-10000;
        maxX=-100000;
        toTranslatableMap(hitboxArray);
        compile();
    }
    ...
    public Array<Integer> analyze_position(Vector3 pos) {
        int x = (int) Math.floor((pos.x - minX-translationX) / tileSize);
        int y = (int) Math.floor((pos.z - minY-translationY) / tileSize);
        if (inBounds(x, y)) {
    ...
    public void translate(float x, float y){
        for(int i=0; i<hitboxArray.size; i++){
            hitboxArray.get(i).resetTranslation();
            hitboxArray.get(i).translate(x,y,0);
        }
        translationX=x;
        translationY=y;
    }
    private void toTranslatableMap(Array<HITBOX> hitboxes){
        this.hitboxArray=new Array<>(hitboxes.size+1);
        for(int i=0; i<hitboxes.size; i++){
            hitboxArray.add(new TranslatableHitbox(hitboxes.get(i)));
        }
    }
}
