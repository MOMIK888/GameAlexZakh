...
public class QuestManager {
    ...
    public QuestManager(){
        if(MainGame.databaseInterface[1].getInfo(0)=="0"){
    ...
    public boolean isQuest(int index){
        for(int i=0; i<completedQuests.size(); i++){
            if((int) completedQuests.get(i)[0]==index){
                return false;
            }
        }
        return true;
    ...
    public void addCompleted(int unique_index){
        completedQuests.add(new Integer[]{unique_index,1});
    }
    public void setQuestsValue(int kill, int collect, int[] tasks){
        ...
        for(int i=0; i<quests.size();i++){
            ...
            if (quest.getIscompleted()) {
                completedQuests.add(new Integer[]{quest.unique_index,1});
                quests.remove(i);
                i--;
            }
        }
    ...
    public String cipherQuestData(){
        String ciph="";
        for(int i=0; i<quests.size(); i++){
            ciph+=quests.get(i).unique_index;
            ciph+="$";
        }
        return ciph;
    }
    public void decipherData(String data){
        if(data==null || data.isEmpty() || data.length()<1 || data==""){
            return;
        }
        String[] newdata=data.split("\\$");
        int[] array=new int[newdata.length];
        for(int i=0; i<newdata.length; i++){
            array[i]=Integer.parseInt(newdata[i]);
        }
        for(int j : array){
            if(j==0) {
                quests.add(new Quest() {
